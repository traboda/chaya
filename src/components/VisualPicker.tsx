...
const buttonClassNames = cva({
  base: ['w-full rounded-lg border', 'transition flex'],
  variants: {
    direction: {
...
const VisualPicker = <Type extends VisualPickerValueType | VisualPickerValueType[]>({
  ...
  useEffect(() => {
    if (isMulti && !Array.isArray(value))
      throw new Error('VisualPicker: value must be an array when isMulti is true');
  }, []);
      ...
      <div
        className={mcs([
          ...
          isVertical
            ? 'grid-cols-1'
            : fitHorizontal
              ? 'grid-cols-flexible-fit'
              : 'grid-cols-flexible-fill',
          className,
        ])}
        style={{
        ...
        {items.map((item, i) => (
          <button
            ...
            className={mcs([
              ...
              isDisabled || item.isDisabled
                ? 'border-neutral-300/50 opacity-90 dark:border-neutral-600/50'
                : 'border-neutral-300/80 bg-background-lighten-1 dark:border-neutral-600/70 dark:bg-background-lighten-2',
              itemClassName,
            ])}
