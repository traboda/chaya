...
const TagSelector = <Type extends SingleValueType | SingleValueType[]>(
  props: TagSelectorProps<Type>
) => {
  const [tag, setTag] = useState<SingleValueType>(Array.isArray(props.value) ? '' : props.value);
  const [tags, setTags] = useState<SingleValueType[]>(
  ...
  const handleTagClick = (tagSelect: OptionType) => {
    ...
    } else if (props.isClearable && tagSelect.value === tag)
      props.onChange(props.options[0].value as Type);
  ...
  const generateClassName = (value: string | number) => {
    ...
    return clsx([
      ...
      className || ['hover:dark:bg-gray-500/30 hover:bg-gray-500/20', 'border-gray-500'],
    ]);
  };
  ...
  return (
    <div id={props?.id} className={mcs(['tag-selector', props?.className])}>
      {props?.labels && (
        <Label htmlFor="" children={props.labels.title} tooltip={props.labels.helpText} />
      )}
