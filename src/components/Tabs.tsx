...
const Tabs = ({
  ...
  const getInitialTab = () => {
    ...
    if (tabItems.length > 0) {
      tabItems.forEach((t) => {
        if ((tabkey === null && t.key) || t.isInitial) tabkey = t.key;
      });
    }
    return tabkey;
  };
  ...
  const verticalSelector = isVertical
    ? useMemo(
        () => (
          <VerticalNavigator
            id={tabID}
  ...
  const horizontalSelector = !isVertical
    ? useMemo(
        () => (
          <HorizontalNavigator
            id={tabID}
  ...
  const ResponsiveView = !disableResponsive
    ? useMemo(
        () => (
          <AccordionGroup
  ...
  return (
    <React.Fragment>
      <div
        className={mcs([
          isVertical ? 'mx-0 flex-wrap' : 'px-0',
      ...
      >
        <div className={clsx([isVertical && 'relative w-full p-0 md:w-1/5'])}>
          {isVertical ? (
            <div className="sticky top-2 w-full">{verticalSelector}</div>
          ) : (
            horizontalSelector
          )}
        </div>
      ...
      {!disableResponsive && <div className="block md:hidden">{ResponsiveView}</div>}
    </React.Fragment>
  );
};
