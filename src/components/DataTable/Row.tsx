...
const stripedRow =
  'dark:odd:bg-neutral-900/50 odd:bg-white dark:even:bg-neutral-900 even:bg-[#f1f3f3]';

const ItemListerItem = <Type extends { id: string }>({
  properties,
  item,
  itemIndex,
  supportAccordion = false,
  ...
  const { isEnabled, selectedIDs, selectItem, isSelected, deselectItem } =
  ...
  const tdClasses = clsx([
    ...
    isPinned
      ...
      : 'dark:group-hover:bg-neutral-700/30 group-hover:bg-gray-300/30',
  ]);
  ...
  return (
    <tr className={clsx(['group', variant === 'striped-row' ? stripedRow : ''])}>
      ...
      {isEnabled && (
        ...
        >
          {isLoading ? (
            <SkeletonItem h="1.25rem" w="1.25rem" />
          ) : (
            <Checkbox
              label=""
              value=""
      ...
      {properties?.length > 0 &&
        properties.map((p, index) => {
          const link = isLoading
            ? null
            : item && typeof p.link === 'function'
              ? p.link(item)
              : null;
          const contentRenderer = isLoading ? (
            <SkeletonItem h="1.75rem" w="80%" />
          ) : (
            item && p.value(item, itemIndex)
          );
          const renderer =
            link || (p?.onClick && typeof p?.onClick === 'function') ? (
              <span className={`group-[${p?.id}-${index}]-row`}>
                {contentRenderer}
                <span className="ml-1 inline-block w-[16px] opacity-0 group-hover:opacity-100">
          ...
          return (
            <td
              ...
              className={mcs([
                ...
                p.stickRight
                  ? index % 2 == 0 && variant === 'striped-column'
                    ? 'bg-neutral-200 dark:bg-neutral-900'
                    : variant == 'striped-row'
                      ? (itemIndex || 1) % 2 != 0
                        ? '!bg-[#f1f3f3] dark:!bg-neutral-900'
